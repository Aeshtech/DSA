...
public class EquilibriumPoint {
    public static int equilibriumPoint(int[] arr){
        ...
        /* Traverse again and firstly subtract the current element from the totalSum and check on each iteration that wheather
           leftSum (initially 0) is equal to the totalSum, if it equal than simply return the index else
        ...
        for(int i=0;i<n;i++){
            ...
            if(leftSum == totalSum)  //check if both leftSum & totalSum are equal
                return i+1;          // +1 for returning readable position
            else
