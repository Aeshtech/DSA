    ...
    public static int[] bubbleSorting(int[] arr){
        ...
        for(int i=0;i<arr.length-1;i++){
            ...
            for(int j=0; j<arr.length-i-1; j++){
                if(arr[j] > arr[j+1]) {    //checking if left sibling is greater than right, then swap these, i.e to move it at last.
                    //--------swapping--------
            ...
            // swapped by inner loop, then break outer loop, its meant our array is sorted now.
            if(swapped != true)
                break;
        }
        return arr;
